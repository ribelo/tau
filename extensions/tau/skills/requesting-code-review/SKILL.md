...
```bash
# Run the project gate or the relevant test suite
npm run gate

# Check for debug code
rg -n "console\.log|debugger" src test
...
Use `agent spawn review` to dispatch a focused reviewer:

```text
agent spawn review "Review implementation for correctness and quality
...
FILES CHANGED:
- src/domain/user.ts (added User type)
- src/http/login.ts (added login handler)
- test/auth/login.test.ts (added 8 tests)
