...
function readSandboxNamespace(settings: any): SandboxConfig {
	// New namespace: settings.tau.sandbox
	const tauSandbox = settings?.tau?.sandbox;
	if (tauSandbox && typeof tauSandbox === "object" && !Array.isArray(tauSandbox)) return tauSandbox as SandboxConfig;
	// Back-compat: settings.sandbox
	const legacy = settings?.sandbox;
	if (legacy && typeof legacy === "object" && !Array.isArray(legacy)) return legacy as SandboxConfig;
	return {};
}
