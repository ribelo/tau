...
const SkillManageParams = Type.Object({
	...
	content: Type.Optional(Type.String({ description: "Full SKILL.md content (YAML frontmatter + markdown body). Required for 'create' and 'edit'." })),
	old_string: Type.Optional(Type.String({ description: "Text to find (required for 'patch'). Must be unique unless replace_all=true." })),
	new_string: Type.Optional(Type.String({ description: "Replacement text (required for 'patch'). Empty string to delete matched text." })),
	replace_all: Type.Optional(Type.Boolean({ description: "For 'patch': replace all occurrences (default: false)." })),
	category: Type.Optional(Type.String({ description: "Optional category for organizing (e.g., 'devops'). Only for 'create'." })),
	file_path: Type.Optional(Type.String({ description: "Path to supporting file (e.g., 'references/api.md'). For write_file/remove_file/patch." })),
	...
	switch (params.action) {
		...
		case "patch": {
			if (params.old_string === undefined) {
				return { ok: false, result: toolFail("old_string is required for 'patch' action.") };
			}
			if (params.new_string === undefined) {
				return { ok: false, result: toolFail("new_string is required for 'patch' action.") };
			}
			const validatedParams: ValidatedSkillManageParams = {
				action: "patch",
				name: params.name,
		...
		case "write_file": {
			if (params.file_path === undefined) {
				return { ok: false, result: toolFail("file_path is required for 'write_file' action.") };
			}
			if (params.file_content === undefined) {
				return { ok: false, result: toolFail("file_content is required for 'write_file' action.") };
			}
			return {
				ok: true,
				params: {
		...
		case "remove_file": {
			if (params.file_path === undefined) {
				return { ok: false, result: toolFail("file_path is required for 'remove_file' action.") };
			}
			return {
				ok: true,
				params: {
		...
		async execute(_toolCallId, rawParams, _signal, _onUpdate, _ctx) {
			...
			const program = Effect.gen(function* () {
				...
				switch (validation.params.action) {
					...
					case "edit": {
						yield* skillManager.edit(validation.params.name, validation.params.content);
					...
					case "delete": {
						yield* skillManager.remove(validation.params.name);
			...
			try {
				...
				if (cause instanceof SkillNotFound) return toolFail(`Skill '${cause.name}' not found.`);
				if (cause instanceof SkillAlreadyExists) return toolFail(`Skill '${cause.name}' already exists at ${cause.path}.`);
				if (cause instanceof SkillInvalidName) return toolFail(`Invalid skill name '${cause.name}': ${cause.reason}`);
				if (cause instanceof SkillInvalidContent) return toolFail(`Invalid skill content: ${cause.reason}`);
				if (cause instanceof SkillFileError) return toolFail(`Skill file error: ${cause.reason}`);
				if (cause instanceof SkillSecurityViolation) return toolFail(`Skill content rejected: ${cause.reason}`);
				if (cause instanceof SkillPatchFailed) return toolFail(`Patch failed for skill '${validation.params.name}': ${cause.reason}`);
